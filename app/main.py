...
def main() -> None:
    st.set_page_config(
        page_title="AI Analytics Assistant for E-commerce",
        layout="wide",
    )

    initialize_session_state()

    st.title("AI Analytics Assistant for E-commerce")
    st.markdown(
        """
    ...
    with st.sidebar:
        ...
        st.write(
            "This app is designed as a mini BI analytics assistant for e-commerce data."
        )
        st.write(
            "It uses an LLM to generate SQL, validates the query, then runs it on PostgreSQL."
        )
